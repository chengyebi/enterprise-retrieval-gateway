...
```sh
# 新 Supabase 项目通常使用 ES256。先下载 JWKS：
curl -s "https://<project>.supabase.co/auth/v1/.well-known/jwks.json" > /tmp/supabase-jwks.json

./build/ergateway serve \
  --backend memory \
  --port 8080 \
  --supabase-jwks-file /tmp/supabase-jwks.json \
...
- `--supabase-jwks-file` 用来验证 Supabase Access Token 的 ES256 签名，适用于使用 JWKS 的 Supabase 项目。
...
- 启用 `--require-supabase-auth` 后，未认证的 `/health` 只返回最小 `{"status":"ok"}`，有效 Bearer token 才能看到 backend 和 corpus 计数。
